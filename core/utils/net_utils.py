...
def train(visualizer, train_loader, model, optimizer, gcn_optimizer, epoch, config, cls_criterion):
    ...
    for i, (input, ema_input, label, flags, name) in enumerate(train_loader):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            # feature_cat_s = StudentFeatureQueue.get_feature()
            # feature_cat_t = TeacherFeatureQueue.get_feature()
            # label_cat, label_mask = TeacherFeatureQueue.get_label()
            if epoch < config['gcn_start_epoch']:
                feature_s = feature_s.detach().clone()
                feature_t = feature_t.detach().clone()
            adjs = feature_s.mm(feature_s.t())
            adjs = torch.norm(adjs, 2, 1)
            output_gcn_s = GCNStudentModel(feature_s, adjs)

            adjt = feature_t.mm(feature_t.t())
            adjt = torch.norm(adjt, 2, 1)
            output_gcn_t = GCNTeacherModel(feature_t, adjt)

            # probe = torch.softmax(output_s, dim=1)
            # probe_t = torch.softmax(output_t, dim=1)
            # adj = torch.matmul(probe, probe_t.T)
            # adj = torch.softmax(adj, dim=1)
            # feature = torch.cat([probe, probe_t], dim=1)
            ...
            # GCN Classification
            gcn_probe = torch.softmax(output_gcn_s, dim=1)
            gcn_cls_loss = cls_criterion(torch.log(gcn_probe[:label_bs]), label[:label_bs])*config['gcn_weight']

            # GCN Classification Consistency
            gcn_consistency_loss_cls = consistency_weight_att * consistency_criterion_cls(output_gcn_s, output_gcn_t)*config['gcn_weight']


            total_loss = loss_cls * cls_loss + (loss_masks * mask_loss + loss_bound * bound_loss)*config['attention_weight'] 

            if epoch >= config['gcn_start_epoch']:
                total_loss = total_loss + consistency_loss_cls + consistency_loss_att + gcn_consistency_loss_cls + src_loss+gcn_cls_loss
...
def valid(valid_loader, model, config):
    ...
    with torch.no_grad():
        ...
        for i, (input, ema_input, label, flags, name) in enumerate(valid_loader):
            ...
            # adj = torch.softmax(adj, dim=1)
            # feature = (feature_s+feature_t)*0.5
            # output_gcn = GCNModel(feature, adj)
            ...
            # probe_t = torch.softmax(output_t, dim=1)
            # adj = torch.matmul(probe_s, probe_t.T)
            # adj = torch.softmax(adj, dim=1)
            # feature = torch.cat([probe_s, probe_t], dim=1)
            # output_gcn = GCNModel(feature, adj)
            ...
            probe_s = torch.softmax(output_s, dim=1)
            probe_t = torch.softmax(output_t, dim=1)
            probe_gcn = torch.softmax(output_gcn_s, dim=1)
