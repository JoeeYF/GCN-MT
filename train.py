...
def arg_parse():
    ...
    parser.add_argument('-cfg', '--config', default='configs/se50_semi.yaml',
...
def main():
    ...
    base_dir = increment_dir('/mnt/data/yuanfang/DC-MT-SRC/outputs/MT', config['description'])
    ...
    GCNTeacherModel = MutilHeadGAT(nfeat=config['gcn_feature_dim'], nhid=config['gcn_hidden_dim'], nclass=len(config['Data_CLASSES']), dropout=config['gcn_dropout'], alpha=0.2, nheads=config['gcn_hidden_num'])
    for param in GCNTeacherModel.parameters():
        param.detach_()
